...
import { Package, Users, FileText, Download, Plus, Trash2, Check, Clock, AlertCircle, ChevronRight, Upload, Copy, Edit2, X, Calendar, ArrowLeft, FileDown, UserCheck } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    loadData();
    // Refrescar cada 30 segundos para ver cambios de otras personas
  ...
  const saveAsesoras = async (newList, action) => {
    // action: { type: 'add'|'delete'|'update', item: {...} }
    try {
      if (action.type === 'add') {
  ...
  const saveSemanas = async (newList, changedSemana) => {
    try {
      if (changedSemana === 'all') {
        // Actualización masiva (desactivar todas + activar nueva)
...
function LogisticaView({ subView, setSubView, setView, asesoras, saveAsesoras, semanas, saveSemanas, semanaActiva, draftWeek, setDraftWeek, showToast, loadData }) {
  return (
